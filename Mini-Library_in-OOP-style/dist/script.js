        ...
        element.childNodes.forEach(function (item) {
          if (item.nodeName === "#text" && item.nodeValue.replace(/\s/g, "").length > 0) {
            if (!item.parentNode.getAttribute("data-fontsize")) {
              var value = window.getComputedStyle(item.parentNode, null).fontSize;
              item.parentNode.setAttribute("data-fontsize", +value.replace(/px/g, ""));
              item.parentNode.style.fontSize = item.parentNode.getAttribute("data-fontsize") * _this2.scale + 'px';
            } else {
              item.parentNode.style.fontSize = item.parentNode.getAttribute("data-fontsize") * _this2.scale + 'px';
            }
          } else {
